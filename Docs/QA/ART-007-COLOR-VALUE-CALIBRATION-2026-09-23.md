...
The current repair canonicalizes source and expected-manifest text line endings to LF before hashing or pin comparison, while keeping generated manifest output deterministic. A CRLF simulation regression proves that a Windows-style checkout produces the same manifest and verifies against a CRLF pin. The verifier now requires exact status `art_reference_source_validated_not_runtime`; a regression repins `runtime_validated_and_approved` and confirms rejection. This stays inside the ART-007 allowlist instead of introducing a repository-wide `.gitattributes` change.
...
The 16-test suite includes absence of a stale source-tree derived pack, fresh-pack equality with the checked-in pin, CRLF-checkout contract portability, valid temporary output, exact regeneration, overwrite refusal, stale-manifest rejection, repinned false runtime-status rejection, contrast rejection, full-pixel semantic corruption in swatch/preview/ramp regions, LUT replacement, oversized-dimension rejection before inflation, corrected vermilion luminance preview, and repinned false UI-screening metadata rejection.
