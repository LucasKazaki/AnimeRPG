...
## Bounded continuation: resize phase deadline, 2026-09-23

Fresh independent review of head `72d8b59724ae6c5a50765520578bc53ba3d6c6d5` identified one additional P2 false-pass in `ResizeAndCheck`: requested dimensions were accepted before checking the existing 1.5-second deadline, nested shell-message validation used only the broader/global message budget, and poll sleeps were not capped to the resize phase deadline. A slow but responsive editor could therefore satisfy the geometry or complete shell validation after the advertised deadline and still pass.

Source repair `ca34b451246f6eb8808fcbcec01dfaf1292df07d` changes only `Tests/EditorRuntimeSmoke.cpp`; new smoke blob `5e4b01f9b755fbeb1688e8600a7f4c85555dd71f`. The repair establishes the deadline before the asynchronous `SetWindowPos`, installs the existing `ScopedMessageDeadline` across the resize polling/validation phase, rejects requested geometry observed at or after the deadline, rechecks after containment and after `ValidateShellState`, and replaces generic polling sleeps with `RemainingDeadlineBudget`. Existing resize geometry, containment, semantic, selection, maximize/restore, exact restore-placement, process-identity, cleanup, and shutdown assertions remain intact. The resolved review thread is `PRRT_kwDOTo2Ig86lU6AM`.

Primary sources rechecked for this continuation:

- Microsoft `SetWindowPos`: https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-setwindowpos . With `SWP_ASYNCWINDOWPOS`, the system can post the request to the window-owning thread instead of blocking the caller, so the acceptance path must positively observe completion inside its own phase budget.
- Microsoft `SendMessageTimeoutW`: https://learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-sendmessagetimeoutw . `uTimeout` bounds an individual cross-thread wait, not the entire resize acceptance workflow, so nested waits must be capped to the remaining phase budget.
- Epic Unreal Engine 5.8 `Using Editor Viewports` and `Viewport Toolbar`: editor viewports include perspective/orthographic workflows, selectable layouts, and maximized/immersive states; these remain comparison workflows only.
- Unity 6.0 `EditorWindow.maximized`: maximized editor state remains a first-class editor-window property.

Public behavioral/API documentation only. No proprietary source copied and no dependency imported.

Disposable source-logic fixture SHA-256 `11cf52f815ceb3a83294c514fa4d2b478d54a3b27c0082651de7e99b71010de0` reproduces both old false-pass cases, geometry first observed at 1501 ms against a 1500 ms deadline and geometry observed at 1400 ms with validation completing at 1600 ms. It also checks remaining-phase budget capping. The fixture passed warning-clean GCC C++17 and Clang C++17 with ASan+UBSan plus leak detection. This is source-logic evidence only, not native Win32 GUI evidence.

Source-associated hosted runs are Windows `35922670722`, profiling `35922670747`, and release-manifest `35922670819`. At this record write, profiling is `success`; Windows and release-manifest are still `in_progress` and are not claimed as passed. Fresh independent review is required for the current source/evidence tree. `native_evidence` remains empty and E11 final acceptance remains false.

Latest observed `main` is `40311ba7dcf1035656b6b2d0dbeb589030071dc7`. Issue #7 remains open, so the historical R0 runner remains blocked and was not invoked.

The single next useful action is to let the current hosted gates and fresh independent review establish the exact current tree, then have the registered Windows executor run Debug/Release `EditorContainmentTests` plus interactive `EditorRuntimeSmoke` on that exact reviewed SHA with the existing machine/toolchain/GPU/desktop/screenshots/process-cleanup evidence, including proof that every resize acceptance completes inside 1.5 seconds and maximize/restore returns to the exact pre-maximize outer rectangle. Separately launch `AstralGame` from the same source/build. Do not start another dependent editor feature before this native gate resolves.
