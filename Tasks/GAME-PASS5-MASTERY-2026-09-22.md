...
- Zenless Zone Zero official current site/version material: https://zenless.hoyoverse.com/en-us/main?catchSpider=1 and Version 3.2 official video/news index. Use ZZZ only as a category reference for enemy traits, stun/setup payoff, and training/challenge structure, not copied kits or content.
...
1. `GAME-022` Attack-profile resistance. Vanguard and Bulwark gain different bounded direct-attack resistances so target selection changes which basic attack is efficient. Reference category: enemy strengths/weaknesses in action RPG combat. Acceptance: Standard remains unchanged; resistance applies once, is visible in damage reports, and never increases damage.
...
- regression coverage for all six increments in the existing compiled `LandmarkEncounterTests` target;
- existing pass-1 through pass-4 behavior must remain covered;
