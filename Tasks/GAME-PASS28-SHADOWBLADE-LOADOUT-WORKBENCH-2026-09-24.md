...
- `Engine/Scene/ShadowbladeActions.h`, only to own/expose the workbench
...
**Acceptance:** saving alone creates no remembered selection; successful apply records it; failed apply preserves it; one-action reapply restores the preset through the existing validated `ApplyPreset()` path.
...
**Adaptation:** each saved slot can receive an optional 24-byte label. Outer ASCII spaces are trimmed, internal spaces are preserved, control characters/empty/overlong labels are rejected atomically, and the label can be cleared without altering the underlying saved preset or current equipment.  
**Acceptance:** `"  Mall Patrol  "` becomes `"Mall Patrol"`; an internal space remains; rejected edits preserve the previous label; empty slots cannot be labeled; clearing the label leaves saved/equipped gameplay state unchanged.
...
2. Pass-28 regressions cover preview non-mutation and exact deltas, focus changes, family filtering, malformed filters, scope divergence, successful/failed preset recall, reapply, label spaces, label rejection atomicity, and clear-label non-mutation.
