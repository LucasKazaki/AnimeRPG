...
- `Engine/Scene/CharacterProgression.h` only for persistent one-time Shadow Crypt reward entitlement; no unrelated progression tuning
...
- `GAME-076`: completion may be replayed; persistent `CharacterProgression` owns the first-clear entitlement and grants 300 XP, 30 mastery points, and 40 enhancement materials once. A checkpoint created before first clear, then resumed after the reward is claimed, cannot roll that entitlement back or duplicate the reward.
- `QOL-016`: `SafeToSuspend()` is true only at active room boundaries; mid-objective snapshots are rejected; resume metadata is versioned and bounded; invalid schema/room/counters/cache relationships fail closed; an active expedition cannot be rewound over live state; persistent reward entitlement is not serialized into rollback-prone room metadata.
...
3. Cover incomplete-lead rejection, duplicate start rejection, each room requirement, local defeat retry, optional-cache gate/idempotency, Bronze/Silver/Gold boundaries, damage saturation, first-clear reward idempotency, replay behavior, checkpoint creation, malformed resume rejection, stale active-run rewind rejection, and the exact pre-clear-checkpoint -> first-clear claim -> stale checkpoint resume -> second completion sequence proving that persistent progression prevents a duplicate reward.
