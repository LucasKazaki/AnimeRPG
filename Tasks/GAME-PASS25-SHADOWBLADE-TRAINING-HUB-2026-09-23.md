...
Independent review found and repaired multiple real defects during this pass. Earlier repairs include exact production `LandmarkEncounter` linkage into the registered aggregation test, player-defeat termination, live timing-preset guidance, finite-target restoration on encounter retry, mismatched retry-owner rejection, stale Ready-state feedback suppression, live queued-threat guidance, exact linked-generation validation, Active-only telemetry sampling, exact cutoff handling, sparse-sample preservation, and authoritative encounter-completion owner provenance.

Exact-head review of `8b8165a6f18a8cadedfd8d5318b814163b81539e` then found three additional P2 defects despite green hosted Windows run #812 and Release-manifest run #591:

- a completed story unlock could still be consumed by an unrelated combat/action pair before the hub established its own binding;
- an out-of-range `DefenseInput` enum could fall through the existing defense implementation as Guard and mutate the run;
- multiple damage deltas inside one 200 ms storage bucket shared the first delta's timestamp, so a later still-fresh delta could expire early at the 20-second cutoff.

The final candidate requires `StartTraining`, and any unlocked encounter retry before hub binding, to use the exact combat/action pair that completed the authoritative encounter. It validates defense input as exactly Guard or Dodge before the practice session can mutate state. Rolling telemetry keeps 128 fixed top-level samples at no more than 5 Hz, but each sample preserves up to 16 exact damage/hit activity timestamps so deltas inside one storage bucket age out independently; pathological over-capacity activity fails closed rather than silently smearing timestamps. Registered regressions cover unrelated-owner start/retry rejection, invalid defense enum rejection, the long-gap stale/fresh case, and a 0.10 s / 0.21 s same-bucket cutoff case observed at 20.15 s.
